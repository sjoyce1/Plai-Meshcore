...
void AdvertTimeHelper::formatRelativeTimeDiff(char dest[], int32_t seconds_from_now, bool short_fmt) {
  ...
  if (seconds_from_now < 60) {
    sprintf(dest, "%d secs %s", (int)seconds_from_now, suffix);
  } else {
    int32_t mins = seconds_from_now / 60;
    if (mins < 60) {
      sprintf(dest, "%d mins %s", (int)mins, suffix);
    } else {
      int32_t hours = mins / 60;
      if (hours < 24) {
        sprintf(dest, "%d hours %s", (int)hours, suffix);
      } else {
        sprintf(dest, "%d days %s", (int)(hours / 24), suffix);
      }
    }
  }
}