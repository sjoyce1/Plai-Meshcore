...
void Dispatcher::checkRecv() {
      ...
      if (pkt == NULL) {
        ...
        if (pkt->path_len > MAX_PATH_SIZE || i + pkt->path_len > len) {
          ...
          if (pkt->payload_len > sizeof(pkt->payload)) {
            MESH_DEBUG_PRINTLN("%s Dispatcher::checkRecv(): packet payload too big, payload_len=%d", getLogDateTime(), (int)pkt->payload_len);
            _mgr->free(pkt);  // put back into pool
            pkt = NULL;  
          } else {
  ...
  if (pkt) {
    ...
    printf("%s: RX, len=%d (type=%d, route=%s, payload_len=%d) SNR=%d RSSI=%d score=%d time=%d", 
    ...
    printf(" hash=");
    mesh::Utils::printHex(packet_hash, MAX_HASH_SIZE);
...
void Dispatcher::checkSend() {
  ...
  if (outbound) {
    ...
    #if MESH_PACKET_LOGGING
      printf("%s: TX, len=%d (type=%d, route=%s, payload_len=%d)", 
...
void Dispatcher::sendPacket(Packet* packet, uint8_t priority, uint32_t delay_millis) {
  if (packet->path_len > MAX_PATH_SIZE || packet->payload_len > MAX_PACKET_PAYLOAD) {
    MESH_DEBUG_PRINTLN("%s Dispatcher::sendPacket(): ERROR: invalid packet... path_len=%d, payload_len=%d", getLogDateTime(), (int) packet->path_len, (int) packet->payload_len);
    _mgr->free(packet);
  } else {
